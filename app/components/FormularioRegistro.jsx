...
export default function FormularioRegistro({ datosIniciales, alGuardar, onFinalizar }) {
  ...
  const [formData, setFormData] = useState({
    ...
    nombre: '',
    apellidos: '',
    telefonoCiclista: '', // Campo nuevo solicitado
    tipoSangre: '',
    seguroMedico: '',
    nss: '',
  ...
  const manejarEnvio = async (e) => {
    ...
    try {
      if (datosIniciales?.id) {
        // ACTUALIZAR (Bloqueamos lógica de nombre/apellidos en el update)
        const docRef = doc(db, "usuarios", datosIniciales.id);
        const { nombre, apellidos, usuario, ...datosActualizables } = formData;
      ...
      if (alGuardar) alGuardar(formData);
    } catch (error) {
  ...
  return (
    <form onSubmit={manejarEnvio} className="p-4 space-y-6 bg-white rounded-[35px]">
      ...
      <div className="flex flex-col items-center space-y-3">
        ...
        <p className="text-[10px] font-black uppercase text-gray-400 tracking-widest text-center">Tu cara de "Hoy se sufre"</p>
      </div>
      ...
      <div className="space-y-4">
        ...
        <div className="bg-orange-50 p-5 rounded-[30px] border-2 border-orange-100 grid grid-cols-2 gap-4">
          ...
          <input type="text" name="usuario" placeholder="Usuario" disabled={esEdicion} value={formData.usuario} onChange={manejarCambio} required className={`bg-transparent border-b-2 border-orange-200 outline-none p-1 font-bold text-black ${esEdicion ? 'opacity-50' : 'focus:border-black'}`} />
          <input type="password" name="password" placeholder="Password" value={formData.password} onChange={manejarCambio} required={!esEdicion} className="bg-transparent border-b-2 border-orange-200 outline-none p-1 font-bold text-black focus:border-black" />
        </div>
        ...
        <div className="bg-white p-5 rounded-[30px] border-2 border-gray-100 space-y-4 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="flex flex-col">
              ...
              <input type="text" name="nombre" disabled={esEdicion} value={formData.nombre} onChange={manejarCambio} required className={`border-b-2 border-gray-100 p-2 outline-none text-black font-bold ${esEdicion ? 'opacity-50' : 'focus:border-orange-500'}`} />
            </div>
            <div className="flex flex-col">
              <label className="text-[9px] font-black text-gray-300 uppercase ml-1">Apellidos</label>
              <input type="text" name="apellidos" disabled={esEdicion} value={formData.apellidos} onChange={manejarCambio} required className={`border-b-2 border-gray-100 p-2 outline-none text-black font-bold ${esEdicion ? 'opacity-50' : 'focus:border-orange-500'}`} />
            </div>
          </div>

          <div className="flex flex-col">
            <label className="text-[9px] font-black text-orange-400 uppercase ml-1">📱 Tu Teléfono Celular</label>
            <input type="tel" name="telefonoCiclista" placeholder="Tu número de contacto" value={formData.telefonoCiclista} onChange={manejarCambio} required className="border-b-2 border-orange-100 p-2 outline-none focus:border-orange-500 text-black font-bold" />
          </div>

          <div className="flex flex-col">
            <label className="text-[9px] font-black text-orange-400 uppercase ml-1">🎂 Fecha de Nacimiento</label>
            <input type="date" name="fechaNacimiento" value={formData.fechaNacimiento} onChange={manejarCambio} required className="w-full bg-orange-50/50 border-b-2 border-orange-100 p-2 rounded-t-lg outline-none focus:border-orange-500 text-black font-bold text-sm" />
          </div>
        </div>
        ...
        <div className="bg-red-50 p-5 rounded-[30px] border-2 border-red-100 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <input type="text" name="contactoEmergenciaNombre" placeholder="Contacto Emergencia" value={formData.contactoEmergenciaNombre} onChange={manejarCambio} required className="bg-transparent border-b-2 border-red-200 outline-none p-1 text-xs text-black font-bold" />
            <input type="text" name="parentesco" placeholder="Parentesco" value={formData.parentesco} onChange={manejarCambio} required className="bg-transparent border-b-2 border-red-200 outline-none p-1 text-xs text-black" />
          </div>
          <div className="grid grid-cols-2 gap-4">
             ...
             <input type="tel" name="contactoEmergenciaTelefono" placeholder="Tel. Emergencia" value={formData.contactoEmergenciaTelefono} onChange={manejarCambio} required className="bg-transparent border-b-2 border-red-200 outline-none p-1 text-xs text-black font-bold" />
          </div>
        </div>
      ...
      <button type="submit" disabled={cargando} className="w-full bg-black text-white font-black py-5 rounded-[25px] hover:bg-orange-600 transition-all shadow-[0_8px_0_0_#444] active:shadow-none active:translate-y-1 uppercase italic tracking-widest text-lg disabled:opacity-50">
        {cargando ? 'SINCRONIZANDO...' : '¡LISTO PARA EL CAFÉ! ☕️🚴‍♂️'}
      </button>
    </form>
  );
}